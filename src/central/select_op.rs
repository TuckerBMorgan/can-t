use crate::central::{equation, get_equation, Shape, Tensor, TensorID};
...
impl Tensor {
    pub fn select(&self, indices_id: TensorID) -> Tensor {
        ...
        let tensor = Tensor::create_tensor_data_and_shape_and_operation(Shape::new(final_shape), result_data, Operation::Select(self.id, indices_id));
        return tensor;
    }
}
...
mod tests {
    ...
    fn select_large_vocabulary_test() {
        ...
        for j in 0..embedding_dim {
            assert!(approx_equal(result_data[[0, 1, j]], (7 * embedding_dim + j) as f32, 1e-6));
        }
        
        // Check batch 1, pos 0 (token 3): [12, 13, 14, 15]
        for j in 0..embedding_dim {
            assert!(approx_equal(result_data[[1, 0, j]], (3 * embedding_dim + j) as f32, 1e-6));
        }
    }

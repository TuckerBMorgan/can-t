...
mod tests {
    ...
    fn test_relu_forward_mixed_values() {
        ...
        for i in 0..5 {
            assert!(
                approx_equal(result[[i]], expected_values[i], 1e-6),
                "ReLU({}) = {}, expected {}",
                if i < 5 {
                    vec![-3.0, -1.5, 0.0, 1.5, 3.0][i]
                } else {
                    0.0
                },
                result[[i]],
    ...
    fn test_relu_forward_matrix() {
        ...
        let mut input = Tensor::from_vec(vec![-1.0, 0.0, 1.0, -0.5, 0.5, 2.0], vec![2, 3]);
        let output = input.relu();
    ...
    fn test_relu_forward_large_values() {
        ...
        for i in 0..6 {
            assert!(
                approx_equal(result[[i]], expected[i], 1e-6),
                "ReLU({}) = {}, expected {}",
                large_values[i],
                result[[i]],
                expected[i]
    ...
    fn test_relu_forward_multiple_calls() {
        ...
        for i in 0..3 {
            assert!(
                approx_equal(result1[[i]], result2[[i]], 1e-10),
    ...
    fn test_relu_backward_mixed_values() {
        ...
        for i in 0..5 {
            assert!(
                approx_equal(grad[[i]], expected_grads[i], 1e-6),
                "Gradient for input {} should be {}, got {}",
                vec![-2.0, -1.0, 0.0, 1.0, 2.0][i],
                expected_grads[i],
                grad[[i]]
            );
        }
    ...
    fn test_relu_backward_gradient_properties() {
        ...
        for i in 0..5 {
            let grad_val = grad[[i]];
            assert!(
                grad_val == 0.0 || grad_val == 1.0,
                "ReLU gradient should be 0 or 1, got {}",
                grad_val
            );
        }
    ...
    fn test_relu_backward_shape_preservation() {
        ...
        let shapes = vec![vec![1], vec![5], vec![2, 3], vec![2, 3, 4]];

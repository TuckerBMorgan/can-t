...
impl Tensor {
    ...
    pub fn softmax(&self, axis: usize) -> Tensor {
        ...
        return Tensor::create_tensor_data_and_shape_and_operation(
            self.shape,
            softmax_internal(&mut self.item(), axis).to_owned().into_raw_vec(),
...
pub fn backward_for_softmax(backprop_packet: BackproagationPacket) {
    if let Operation::Softmax(source_id, axis) = backprop_packet.operation {
        ...
        let softmax_output = softmax_internal(&mut backprop_packet.equation.get_item(source_id),axis );
        ...
        // 6. Add to gradients
        backprop_packet.equation.add_tensor_grad(source_id, gradients.into_raw_vec());
    } else {

...
impl Tensor {
    ...
    pub fn std(&self, axes: Vec<usize>) -> Tensor {
        ...
        let (data, output_shape) = {
            ...
            let broadcasted = mean_as_ndarray
                .broadcast(equation.get_tensor_shape(self.id).as_ndarray_shape())
                .unwrap()
                .to_owned();
        ...
        return Tensor::create_tensor_data_and_shape_and_operation(
            ...
            Operation::Std(self.id, copied_axes, axes.len()),
        );
    }
}
...
pub fn backward_for_std(backprop_packet: BackproagationPacket) {
    if let Operation::Std(source_id, axes_array, num_axes) = backprop_packet.operation {
        ...
        let mean_vec: Vec<f32> = mean_tensor
            .iter()
            .map(|&x| x / total_count as f32)
            .collect();
        ...
        let broadcasted_mean = mean_array
            .broadcast(source_shape.as_ndarray_shape())
            .unwrap();

        // Compute std (replicating forward pass)
        let variance_data: Vec<f32> = source_data
            .iter()
        ...
        let mut var_tensor =
            ArrayD::from_shape_vec(source_shape.dimensions(), variance_data).unwrap();
        for (index, &axis) in axes.iter().enumerate() {
        ...
        let broadcasted_dout = grad_array
            .broadcast(source_shape.as_ndarray_shape())
            .unwrap();

        // Apply gradient formula: (∂L/∂σ) * (1/σ) * (1/N) * (x_i - μ)
        let gradients: Vec<f32> = source_data
            .iter()
            .zip(broadcasted_mean.iter())
        ...
        backprop_packet
            .equation
            .add_tensor_grad(source_id, gradients);
    } else {
...
#[cfg(test)]
mod tests {
    use crate::central::{get_equation, Operation, Shape, Tensor};
    ...
    */
    ...
    fn std_2d_axis0_backward_test() {
        ...
        assert!(approx_equal(
            gradients[[0, 0]],
            (1.0 / std_val) * (1.0 / n) * (1.0 - 2.5),
            1e-5
        ));
        assert!(approx_equal(
            gradients[[0, 1]],
            (1.0 / std_val) * (1.0 / n) * (2.0 - 3.5),
            1e-5
        ));
        assert!(approx_equal(
            gradients[[0, 2]],
            (1.0 / std_val) * (1.0 / n) * (3.0 - 4.5),
            1e-5
        ));
        assert!(approx_equal(
            gradients[[1, 0]],
            (1.0 / std_val) * (1.0 / n) * (4.0 - 2.5),
            1e-5
        ));
        assert!(approx_equal(
            gradients[[1, 1]],
            (1.0 / std_val) * (1.0 / n) * (5.0 - 3.5),
            1e-5
        ));
        assert!(approx_equal(
            gradients[[1, 2]],
            (1.0 / std_val) * (1.0 / n) * (6.0 - 4.5),
            1e-5
        ));
    }
    ...
    fn std_2d_axis1_backward_test() {
        ...
        assert!(approx_equal(
            gradients[[0, 0]],
            (1.0 / std_val) * (1.0 / n) * (1.0 - 2.0),
            1e-4
        ));
        assert!(approx_equal(
            gradients[[0, 1]],
            (1.0 / std_val) * (1.0 / n) * (2.0 - 2.0),
            1e-4
        ));
        assert!(approx_equal(
            gradients[[0, 2]],
            (1.0 / std_val) * (1.0 / n) * (3.0 - 2.0),
            1e-4
        ));
        ...
        assert!(approx_equal(
            gradients[[1, 0]],
            (1.0 / std_val) * (1.0 / n) * (4.0 - 5.0),
            1e-4
        ));
        assert!(approx_equal(
            gradients[[1, 1]],
            (1.0 / std_val) * (1.0 / n) * (5.0 - 5.0),
            1e-4
        ));
        assert!(approx_equal(
            gradients[[1, 2]],
            (1.0 / std_val) * (1.0 / n) * (6.0 - 5.0),
            1e-4
        ));
    }
    ...
    fn std_3d_backward_test() {
        ...
        assert!(approx_equal(
            gradients[[0, 0, 0]],
            (1.0 / std_val) * (1.0 / n) * (1.0 - 3.0),
            1e-5
        ));
        assert!(approx_equal(
            gradients[[0, 0, 1]],
            (1.0 / std_val) * (1.0 / n) * (2.0 - 4.0),
            1e-5
        ));
        assert!(approx_equal(
            gradients[[0, 1, 0]],
            (1.0 / std_val) * (1.0 / n) * (3.0 - 5.0),
            1e-5
        ));
        assert!(approx_equal(
            gradients[[0, 1, 1]],
            (1.0 / std_val) * (1.0 / n) * (4.0 - 6.0),
            1e-5
        ));
        ...
        assert!(approx_equal(
            gradients[[1, 0, 0]],
            (1.0 / std_val) * (1.0 / n) * (5.0 - 3.0),
            1e-5
        ));
        assert!(approx_equal(
            gradients[[1, 0, 1]],
            (1.0 / std_val) * (1.0 / n) * (6.0 - 4.0),
            1e-5
        ));
        assert!(approx_equal(
            gradients[[1, 1, 0]],
            (1.0 / std_val) * (1.0 / n) * (7.0 - 5.0),
            1e-5
        ));
        assert!(approx_equal(
            gradients[[1, 1, 1]],
            (1.0 / std_val) * (1.0 / n) * (8.0 - 6.0),
            1e-5
        ));
    }

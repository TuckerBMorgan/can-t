...
impl Tensor {
    ...
    pub fn einsum(
                    ...
                    match lhs_char {
                        ...
                        ',' => {
                            ...
                            if current_operand < number_of_operands {
                                panic!("fewer opearands where provided that exists in the input string");
                            }
                        ...
                        _ => {
                            ...
                            operand_labels[current_operand].push(label_to_subscript(lhs_char as u8));
                        }
                    }
                },
                _ => {
        ...
        assert!(current_operand == number_of_operands - 1, "Improper number of operands passed to einsum");
  ...
  //          let mut has_ellipsis = false;
            ...
            for label in labels {
                if *label == ELLIPSIS {
                    number_labels -= 1;
//                    has_ellipsis = true;
                    number_of_ellipsis_dimensions = number_of_ellipsis_dimensions.max(number_of_dims.number_of_dimension() - number_labels);
                }
                else {
                ...
                match next {
                    Some(rhs_char) => {
                        match rhs_char {
                            ...
                            '.' => {
                                assert!(ellipsis_in_output == false, "Double Ellipsis found in output");
                                // Are there enough characters left in the string
                            ...
                            _ => {
                                ...
                                assert!(label_count[index as usize] > 0 && permanent_label_index[index as usize] == -1, "index appears more then once in the output index");
        ...
        for i in 0..number_of_operands {
            ...
            for s in &operand_labels[i] {
                if *s == ELLIPSIS {
                    let number_of_dimensions = operands[i].shape.number_of_dimension() - operand_labels[i].len() - 1;
                    for ii in (number_of_ellipsis_dimensions - number_of_dimensions)..number_of_ellipsis_dimensions {

                        if operand.shape.dimensions()[dimension] != 1 {

                            assert!(size_of_ellipsis[ii] == 1 || size_of_ellipsis[ii] == operand.shape.dimensions()[dimension]);

                            size_of_ellipsis[ii] = operand.shape.dimensions()[dimension];
                            dimenion_count[index_of_ellipsis + ii] = dimenion_count[index_of_ellipsis + ii] + 1;
                        }
                    ...
                    if operand.shape.dimensions()[dimension] != 1 {
                        assert!(label_size[*s as usize] == 1 || label_size[*s as usize] == operand.shape.dimensions()[dimension]);
                        label_size[*s as usize] = operand.shape.dimensions()[dimension];
                        dimenion_count[permanent_label_index[*s as usize] as usize] = dimenion_count[permanent_label_index[*s as usize] as usize] + 1;
                    }
                    permutation[label_size[*s as usize]] = dimension as isize;
                    dimension += 1;
                }
                else {
                    let previous_dimension = permutation[permanent_label_index[*s as usize] as usize];

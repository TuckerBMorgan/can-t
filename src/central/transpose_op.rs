...
impl Tensor {
    ...
    fn swap_axes(data: &[f32], shape: &Vec<usize>, axis1: usize, axis2: usize) -> Vec<f32> {
        ...
        for i in (0..ndims - 1).rev() {
            ...
            swap_strides[i] = swap_strides[i + 1]
                * shape[if i + 1 == axis1 {
                    axis2
                } else if i + 1 == axis2 {
                    axis1
    ...
    pub fn transpose(&self, first_index: usize, second_index: usize) -> Tensor {
        let data = {
            ...
            let swapped_data =
                Tensor::swap_axes(data_as_flat_buffer, &shape, first_index, second_index);
            swapped_data
        };
        ...
        let tensor = Tensor::create_tensor_data_and_shape_and_operation(
            Shape::new(shape),
            data,
            super::Operation::Transpose(self.id, first_index, second_index),
        );

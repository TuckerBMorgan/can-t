...
impl Tensor {
    ...
    pub fn tanh(&self) -> Tensor {
        let result : Vec<f32> = get_equation().get_data_flat_buffer(self.id).iter().map(|x|x.tanh()).collect();
        let tensor = Tensor::create_tensor_data_and_shape_and_operation(self.shape, result, Operation::Tanh(self.id));
        return tensor;
    }
}
...
pub fn backward_for_tanh(backprop_packet: BackproagationPacket) {
    if let Operation::Tanh(source_id) = backprop_packet.operation {
        let in_gradient = backprop_packet.equation.get_grad_flat_buffer(backprop_packet.incoming_grad);
        ...
        let updated : Vec<f32> = in_gradient.iter().zip(source_data.iter()).map(|(grad, &x)| grad * (1.0 - x.tanh().powf(2.0))).collect();
...
mod tests {
    ...
    fn tanh_extreme_values_backward_test() {
        ...
        // For x = 0, tanh'(0) = 1
        assert!(approx_equal(gradients[0], 1.0 - (10.0_f32.tanh().powf(2.0)), 1e-6)); // Very small
        assert!(approx_equal(gradients[1], 1.0 - ((-10.0_f32).tanh().powf(2.0)), 1e-6)); // Very small  

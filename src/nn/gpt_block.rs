...
mod tests {
    ...
    fn test_gpt2_block_backward_residual_connections() {
        ...
        let mut input = Tensor::from_vec(vec![1.0; batch_size * seq_len * embed_dim], vec![batch_size, seq_len, embed_dim]);
        input.set_requires_grad(true);
        ...
        assert!(has_nonzero_input_grad, "Input should receive gradients through residual connections");
    ...
    fn test_gpt2_block_backward_attention_gradients() {
        ...
        assert!(input_grad.iter().any(|&g| g.abs() > 1e-6), "Should have non-trivial gradients from attention");
    ...
    fn test_gpt2_block_backward_sequence_length_scaling() {
        ...
        for seq_len in seq_lengths {
            ...
            assert!(input_grad.iter().all(|&g| g.is_finite()), "All gradients should be finite for seq_len {}", seq_len);
            assert!(input_grad.iter().any(|&g| g.abs() > 1e-8), "Should have non-zero gradients for seq_len {}", seq_len);
        }
    }
    ...
    fn test_gpt2_block_backward_chain_rule() {
        ...
        let mut input = Tensor::from_vec(vec![0.3; batch_size * seq_len * embed_dim], vec![batch_size, seq_len, embed_dim]);
        input.set_requires_grad(true);
        ...
        assert!(input_grad.iter().any(|&g| g.abs() > 1e-6), "Chain rule should propagate gradients");
        
        // Gradients should reflect the 2x scaling from the transformation
        let avg_grad_magnitude: f32 = input_grad.iter().map(|&g| g.abs()).sum::<f32>() / input_grad.len() as f32;
        assert!(avg_grad_magnitude > 1e-4, "Gradients should reflect chain rule scaling");
    }
}
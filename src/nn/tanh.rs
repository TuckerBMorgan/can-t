...
mod tests {
    ...
    fn test_tanh_layer_forward_known_values() {
        ...
        for (input_val, expected) in test_cases {
            ...
            let result = output.item();
            assert!(
                approx_equal(result[[0]], expected, 1e-5),
                "tanh({}) = {}, expected {}",
                input_val,
                result[[0]],
                expected
    ...
    fn test_tanh_layer_forward_matrix() {
        ...
        let input = Tensor::from_vec(vec![-1.0, 0.0, 1.0, -0.5, 0.5, 2.0], vec![2, 3]);
        let output = layer.forward(input);
    ...
    fn test_tanh_layer_antisymmetry() {
        ...
        for &val in &values {
            ...
            assert!(
                approx_equal(pos_result, -neg_result, 1e-6),
    ...
    fn test_tanh_layer_zero_gradient_at_saturation() {
        ...
        assert!(
            grad[[0]].abs() < 0.01,
            "Gradient at saturation should be small, got {}",
            grad[[0]]
        );
    }
    ...
    fn test_tanh_layer_gradient_at_zero() {
        ...
        assert!(
            approx_equal(grad[[0]], 1.0, 1e-5),
            "Gradient of tanh at 0 should be 1.0, got {}",
            grad[[0]]
        );
    }

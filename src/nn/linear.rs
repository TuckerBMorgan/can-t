...
mod tests {
    ...
    fn test_linear_backward_with_relu_activation() {
        ...
        assert!(
            has_nonzero_grads,
            "Should have some non-zero gradients despite ReLU"
        );
    }
    ...
    fn test_linear_backward_deep_network() {
        ...
        layer1.weights = Tensor::from_vec(
        ...
        layer2.weights = Tensor::from_vec(
            (0..12).map(|i| (i as f32) * 0.08 - 0.5).collect(),
            vec![4, 3],
        );
        layer2.weights.set_requires_grad(true);
        ...
        for (layer, name) in layers_and_names {
            ...
            assert!(
                ...
                "{} weights should have finite gradients",
                name
            );
            ...
            assert!(
                ...
                "{} bias should have finite gradients",
                name
            );
            ...
            assert!(
                ...
                "{} should have some significant gradients",
                name
            );
        }
    }
    ...
    fn test_linear_backward_different_loss_functions() {
        ...
        let weight_grads_differ = weight_grad1
            .iter()
            .zip(weight_grad2.iter())
            .any(|(g1, g2)| (g1 - g2).abs() > 1e-6);
        let bias_grads_differ = bias_grad1
            .iter()
            .zip(bias_grad2.iter())
            .any(|(g1, g2)| (g1 - g2).abs() > 1e-6);

        assert!(
            weight_grads_differ || bias_grads_differ,
    ...
    fn test_linear_backward_gradient_accumulation() {
        ...
        let bias_grad_after_first = layer
            .bias
            .as_ref()
            .unwrap()
            .grad()
        ...
        let bias_grad_after_second = layer
            .bias
            .as_ref()
            .unwrap()
            .grad()
        ...
        for i in 0..weight_grad_after_first.len() {
            assert!(
                ...
                weight_grad_after_second[i],
                weight_grad_after_first[i]
            );
        }
        ...
        for i in 0..bias_grad_after_first.len() {
            assert!(
                ...
                bias_grad_after_second[i],
                bias_grad_after_first[i]
            );
        }
    }
}

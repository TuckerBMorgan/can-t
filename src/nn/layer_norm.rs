...
mod tests {
    ...
    fn test_layer_norm_forward_single_sample_pytorch_comparison() {
        ...
        let mut gguf_file = GGUFFile::new(String::from("./models/tests/layer_norm/layer_norm_single_sample.gguf"));
        
        // Load PyTorch reference data
        let pytorch_input = Tensor::from_gguf_file(String::from("layer_norm_single_sample_input"), &mut gguf_file);
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_single_sample_output"), &mut gguf_file);
        let pytorch_weight = Tensor::from_gguf_file(String::from("layer_norm_single_sample_weight"), &mut gguf_file);
        let pytorch_bias = Tensor::from_gguf_file(String::from("layer_norm_single_sample_bias"), &mut gguf_file);
        ...
        // Compare outputs
        assert_eq!(rust_output.shape.dimensions(), pytorch_output.shape.dimensions());
        ...
        for i in 0..1 {
            for j in 0..4 {
                ...
                assert!(
                    ...
                    i, j, rust_val, pytorch_val, (rust_val - pytorch_val).abs()
                );
            }
        }
        
        println!("✓ Rust LayerNorm matches PyTorch LayerNorm output within {:.1e} tolerance", epsilon);
    }
    ...
    fn test_layer_norm_forward_zero_variance_pytorch_comparison() {
        ...
        let mut gguf_file = GGUFFile::new(String::from("./models/tests/layer_norm/layer_norm_zero_variance.gguf"));
        
        // Load PyTorch reference data
        let pytorch_input = Tensor::from_gguf_file(String::from("layer_norm_zero_variance_input"), &mut gguf_file);
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_zero_variance_output"), &mut gguf_file);
        let pytorch_weight = Tensor::from_gguf_file(String::from("layer_norm_zero_variance_weight"), &mut gguf_file);
        let pytorch_bias = Tensor::from_gguf_file(String::from("layer_norm_zero_variance_bias"), &mut gguf_file);
        ...
        for i in 0..1 {
            for j in 0..4 {
                ...
                assert!(rust_val.abs() < 0.1, "Rust output should be very small: {:.6}", rust_val);
                assert!(pytorch_val.abs() < 0.1, "PyTorch output should be very small: {:.6}", pytorch_val);
            }
        }
        
        println!("✓ Rust LayerNorm matches PyTorch LayerNorm zero variance behavior");
        println!("  Both produce near-zero outputs for uniform input, confirming gradient vanishing issue");
    }
    ...
    fn test_layer_norm_forward_batch_pytorch_comparison() {
        ...
        let mut gguf_file = GGUFFile::new(String::from("./models/tests/layer_norm/layer_norm_batch.gguf"));
        
        // Load PyTorch reference data
        let pytorch_input = Tensor::from_gguf_file(String::from("layer_norm_batch_input"), &mut gguf_file);
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_batch_output"), &mut gguf_file);
        let pytorch_weight = Tensor::from_gguf_file(String::from("layer_norm_batch_weight"), &mut gguf_file);
        let pytorch_bias = Tensor::from_gguf_file(String::from("layer_norm_batch_bias"), &mut gguf_file);
        ...
        // Compare each element
        for i in 0..2 { // batch size
            for j in 0..3 { // features
                let rust_val = rust_result[[i, j]];
                let pytorch_val = pytorch_result[[i, j]];
                assert!(
                    approx_equal(rust_val, pytorch_val, epsilon),
        ...
        for batch_idx in 0..2 {
            ...
            let sample_sum = rust_result[[batch_idx, 0]] + rust_result[[batch_idx, 1]] + rust_result[[batch_idx, 2]];
            let sample_mean = sample_sum / 3.0;
            println!("  Sample {} output mean: {:.6}", batch_idx + 1, sample_mean);
            assert!(approx_equal(sample_mean, 0.0, 1e-4), "Sample {} mean should be ~0: {:.6}", batch_idx + 1, sample_mean);
            
            // Check all values are finite
            for feature_idx in 0..3 {
                assert!(rust_result[[batch_idx, feature_idx]].is_finite(), 
                    "Sample {} feature {} should be finite", batch_idx + 1, feature_idx + 1);
            }
        }
        ...
        println!("  Sample 1 pattern: [{:.4}, {:.4}, {:.4}]", rust_result[[0, 0]], rust_result[[0, 1]], rust_result[[0, 2]]);
        println!("  Sample 2 pattern: [{:.4}, {:.4}, {:.4}]", rust_result[[1, 0]], rust_result[[1, 1]], rust_result[[1, 2]]);
    ...
    fn test_layer_norm_forward_known_values_pytorch_comparison() {
        ...
        let mut gguf_file = GGUFFile::new("./models/tests/layer_norm/layer_norm_forward_known_values.gguf".to_string());
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_forward_known_values_output"), &mut gguf_file);
        ...
        assert!(approx_equal(rust_output[[0, 0]], pytorch_output_data[[0, 0]], 1e-4));
        assert!(approx_equal(rust_output[[0, 1]], pytorch_output_data[[0, 1]], 1e-4));
    ...
    fn test_layer_norm_backward_zero_variance_pytorch_comparison() {
        ...
        let mut gguf_file = GGUFFile::new(String::from("./models/tests/layer_norm/layer_norm_backward_zero_variance.gguf"));
        
        // Load PyTorch reference data
        let pytorch_input = Tensor::from_gguf_file(String::from("layer_norm_backward_zero_variance_input"), &mut gguf_file);
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_backward_zero_variance_output"), &mut gguf_file);
        let pytorch_weight = Tensor::from_gguf_file(String::from("layer_norm_backward_zero_variance_weight"), &mut gguf_file);
        let pytorch_bias = Tensor::from_gguf_file(String::from("layer_norm_backward_zero_variance_bias"), &mut gguf_file);
        let pytorch_input_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_zero_variance_input_grad"), &mut gguf_file);
        let pytorch_weight_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_zero_variance_weight_grad"), &mut gguf_file);
        let pytorch_bias_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_zero_variance_bias_grad"), &mut gguf_file);
        ...
        assert!(max_input_grad < 1e-3, "Input gradients should be very small: {:.6}", max_input_grad);
        assert!(max_weight_grad < 1e-3, "Weight gradients should be very small: {:.6}", max_weight_grad);
        assert!(max_bias_grad > 0.5, "Bias gradients should be significant: {:.6}", max_bias_grad);
        
        println!("✓ Rust LayerNorm backward matches PyTorch LayerNorm backward zero variance behavior");
    ...
    fn test_layer_norm_backward_simple_pytorch_comparison() {
        ...
        // Load PyTorch test data from GGUF
        let mut gguf_file = GGUFFile::new("./models/tests/layer_norm/layer_norm_backward_simple.gguf".to_string());
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_backward_simple_output"), &mut gguf_file);
        let pytorch_weight_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_simple_weight_grad"), &mut gguf_file);
        let pytorch_bias_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_simple_bias_grad"), &mut gguf_file);
        ...
        assert!(approx_equal(weight_grad[[0]], pytorch_weight_grad_data[[0]], 1e-4));
        assert!(approx_equal(weight_grad[[1]], pytorch_weight_grad_data[[1]], 1e-4));
        
        // Compare bias gradients  
        assert!(approx_equal(bias_grad[[0]], pytorch_bias_grad_data[[0]], 1e-4));
        assert!(approx_equal(bias_grad[[1]], pytorch_bias_grad_data[[1]], 1e-4));
    ...
    fn test_layer_norm_backward_batch_pytorch_comparison() {
        ...
        // Load PyTorch test data from GGUF
        let mut gguf_file = GGUFFile::new("./models/tests/layer_norm/layer_norm_backward_batch.gguf".to_string());
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_backward_batch_output"), &mut gguf_file);
        let pytorch_input_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_batch_input_grad"), &mut gguf_file);
        let pytorch_weight_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_batch_weight_grad"), &mut gguf_file);
        let pytorch_bias_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_batch_bias_grad"), &mut gguf_file);
        ...
        for i in 0..2 {
            for j in 0..3 {
                assert!(
                    ...
                    i, j, rust_output[[i, j]], pytorch_output_data[[i, j]]
                );
            }
        }
        ...
        for i in 0..2 {
            for j in 0..3 {
                assert!(
                    approx_equal(rust_input_grad[[i, j]], pytorch_input_grad_data[[i, j]], 1e-4),
                    "Input gradient mismatch at [{}, {}]: rust={}, pytorch={}",
                    i, j, rust_input_grad[[i, j]], pytorch_input_grad_data[[i, j]]
                );
            }
        }
        ...
        for i in 0..3 {
            assert!(
                ...
                i, rust_weight_grad[[i]], pytorch_weight_grad_data[[i]]
            );
        }
        ...
        for i in 0..3 {
            assert!(
                ...
                i, rust_bias_grad[[i]], pytorch_bias_grad_data[[i]]
            );
        }
    }
    ...
    fn test_layer_norm_forward_large_values_pytorch_comparison() {
        ...
        // Load PyTorch test data from GGUF
        let mut gguf_file = GGUFFile::new("./models/tests/layer_norm/layer_norm_forward_large_values.gguf".to_string());
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_forward_large_values_output"), &mut gguf_file);
        let pytorch_mean = Tensor::from_gguf_file(String::from("layer_norm_forward_large_values_mean"), &mut gguf_file);
        let pytorch_variance = Tensor::from_gguf_file(String::from("layer_norm_forward_large_values_variance"), &mut gguf_file);
        let pytorch_std = Tensor::from_gguf_file(String::from("layer_norm_forward_large_values_std"), &mut gguf_file);
        ...
        for i in 0..3 {
            assert!(
                ...
                i, rust_output[[0, i]], pytorch_output_data[[0, i]]
            );
        }
        
        // Verify numerical stability - all values should be finite and reasonable
        for i in 0..3 {
            assert!(rust_output[[0, i]].is_finite(), "Output should be finite at [0, {}]", i);
            assert!(rust_output[[0, i]].abs() < 10.0, "Output should be reasonable at [0, {}]: {}", i, rust_output[[0, i]]);
        }
        
        // Check normalization worked (mean should be ~0)
        let mean = (rust_output[[0, 0]] + rust_output[[0, 1]] + rust_output[[0, 2]]) / 3.0;
        assert!(approx_equal(mean, 0.0, 1e-3), "Output mean should be ~0: {}", mean);
        ...
        assert!(approx_equal(pytorch_mean_data[[0]], 200.0, 1e-4), "PyTorch mean should be 200: {}", pytorch_mean_data[[0]]);
        assert!(approx_equal(pytorch_variance_data[[0]], 6666.67, 1.0), "PyTorch variance should be ~6666.67: {}", pytorch_variance_data[[0]]);
        assert!(approx_equal(pytorch_std_data[[0]], 81.65, 0.1), "PyTorch std should be ~81.65: {}", pytorch_std_data[[0]]);
    }
    ...
    fn test_layer_norm_backward_numerical_stability_pytorch_comparison() {
        ...
        // Load PyTorch test data from GGUF
        let mut gguf_file = GGUFFile::new("./models/tests/layer_norm/layer_norm_backward_numerical_stability.gguf".to_string());
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_backward_numerical_stability_output"), &mut gguf_file);
        let pytorch_input_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_numerical_stability_input_grad"), &mut gguf_file);
        let pytorch_weight_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_numerical_stability_weight_grad"), &mut gguf_file);
        let pytorch_bias_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_numerical_stability_bias_grad"), &mut gguf_file);
        ...
        for i in 0..4 {
            assert!(
                approx_equal(rust_input_grad[[0, i]], pytorch_input_grad_data[[0, i]], 1e-4),
                "Input gradient mismatch at [0, {}]: rust={}, pytorch={}",
                i, rust_input_grad[[0, i]], pytorch_input_grad_data[[0, i]]
            );
        }
        ...
        for i in 0..4 {
            assert!(rust_weight_grad[[i]].is_finite(), "Weight gradient should be finite for large values");
            assert!(rust_bias_grad[[i]].is_finite(), "Bias gradient should be finite for large values");
        }
        
        // Additional numerical stability checks
        for &grad in rust_input_grad.iter() {
            assert!(grad.abs() < 1e10, "Input gradient should be reasonable: {}", grad);
        }
        
        for i in 0..4 {
            assert!(rust_weight_grad[[i]].abs() < 1e10, "Weight gradient should be reasonable: {}", rust_weight_grad[[i]]);
            assert!(rust_bias_grad[[i]].abs() < 1e10, "Bias gradient should be reasonable: {}", rust_bias_grad[[i]]);
        }
    }
    ...
    fn test_layer_norm_forward_preserves_batch_independence_pytorch_comparison() {
        ...
        // Load PyTorch test data from GGUF
        let mut gguf_file = GGUFFile::new("./models/tests/layer_norm/layer_norm_forward_preserves_batch_independence.gguf".to_string());
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_forward_preserves_batch_independence_output"), &mut gguf_file);
        let pytorch_sample1_mean = Tensor::from_gguf_file(String::from("layer_norm_forward_preserves_batch_independence_sample1_mean"), &mut gguf_file);
        let pytorch_sample1_var = Tensor::from_gguf_file(String::from("layer_norm_forward_preserves_batch_independence_sample1_var"), &mut gguf_file);
        let pytorch_sample2_mean = Tensor::from_gguf_file(String::from("layer_norm_forward_preserves_batch_independence_sample2_mean"), &mut gguf_file);
        let pytorch_sample2_var = Tensor::from_gguf_file(String::from("layer_norm_forward_preserves_batch_independence_sample2_var"), &mut gguf_file);
        ...
        // Sample 1: [1, 2] -> mean=1.5, centered=[-0.5, 0.5], normalized=[-1, 1]
        assert!(approx_equal(rust_output[[0, 0]], -1.0, 1e-4), "Sample 1 first element should be -1: {}", rust_output[[0, 0]]);
        assert!(approx_equal(rust_output[[0, 1]], 1.0, 1e-4), "Sample 1 second element should be 1: {}", rust_output[[0, 1]]);
        
        // Sample 2: [10, 20] -> mean=15, centered=[-5, 5], normalized=[-1, 1]
        assert!(approx_equal(rust_output[[1, 0]], -1.0, 1e-4), "Sample 2 first element should be -1: {}", rust_output[[1, 0]]);
        assert!(approx_equal(rust_output[[1, 1]], 1.0, 1e-4), "Sample 2 second element should be 1: {}", rust_output[[1, 1]]);
        ...
        assert!(approx_equal(pytorch_sample1_mean_data[[0]], 1.5, 1e-4), "Sample 1 mean should be 1.5: {}", pytorch_sample1_mean_data[[0]]);
        assert!(approx_equal(pytorch_sample1_var_data[[0]], 0.25, 1e-4), "Sample 1 variance should be 0.25: {}", pytorch_sample1_var_data[[0]]);
        
        // Sample 2: [10, 20] -> mean=15, variance=25
        assert!(approx_equal(pytorch_sample2_mean_data[[0]], 15.0, 1e-4), "Sample 2 mean should be 15.0: {}", pytorch_sample2_mean_data[[0]]);
        assert!(approx_equal(pytorch_sample2_var_data[[0]], 25.0, 1e-4), "Sample 2 variance should be 25.0: {}", pytorch_sample2_var_data[[0]]);
        ...
        for i in 0..2 {
            for j in 0..2 {
                assert!(rust_output[[i, j]].is_finite(), "Output should be finite at [{}, {}]", i, j);
            }
        }
    }
    ...
    fn test_layer_norm_forward_negative_values_pytorch_comparison() {
        ...
        // Load PyTorch test data from GGUF
        let mut gguf_file = GGUFFile::new("./models/tests/layer_norm/layer_norm_forward_negative_values.gguf".to_string());
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_forward_negative_values_output"), &mut gguf_file);
        let pytorch_mean = Tensor::from_gguf_file(String::from("layer_norm_forward_negative_values_mean"), &mut gguf_file);
        let pytorch_variance = Tensor::from_gguf_file(String::from("layer_norm_forward_negative_values_variance"), &mut gguf_file);
        let pytorch_std = Tensor::from_gguf_file(String::from("layer_norm_forward_negative_values_std"), &mut gguf_file);
        ...
        // Check normalization worked (mean should be ~0)
        let mean = (rust_output[[0, 0]] + rust_output[[0, 1]] + rust_output[[0, 2]]) / 3.0;
        assert!(approx_equal(mean, 0.0, 1e-4), "Output mean should be ~0: {}", mean);
        ...
        assert!(approx_equal(pytorch_mean_data[[0]], -1.0, 1e-4), "Mean should be -1.0: {}", pytorch_mean_data[[0]]);
        assert!(approx_equal(pytorch_variance_data[[0]], 0.6666666865348816, 1e-4), "Variance should be ~0.667: {}", pytorch_variance_data[[0]]);
        assert!(approx_equal(pytorch_std_data[[0]], 0.8165027499198914, 1e-4), "Std should be ~0.816: {}", pytorch_std_data[[0]]);
        
        // Verify expected output values: [-1.225, 0, 1.225]
        assert!(approx_equal(rust_output[[0, 0]], -1.2247356176376343, 1e-4), "First output should be ~-1.225: {}", rust_output[[0, 0]]);
        assert!(approx_equal(rust_output[[0, 1]], 0.0, 1e-4), "Second output should be ~0: {}", rust_output[[0, 1]]);
        assert!(approx_equal(rust_output[[0, 2]], 1.2247356176376343, 1e-4), "Third output should be ~1.225: {}", rust_output[[0, 2]]);
        ...
        assert!(rust_output[[0, 0]] < 0.0, "First output should be negative (input was most negative)");
        assert!(approx_equal(rust_output[[0, 1]], 0.0, 1e-4), "Second output should be zero (input was mean)");
        assert!(rust_output[[0, 2]] > 0.0, "Third output should be positive (input was most positive)");
    }
    ...
    fn test_layer_norm_forward_different_feature_sizes_pytorch_comparison() {
        ...
        let mut gguf_file = GGUFFile::new("./models/tests/layer_norm/layer_norm_forward_different_feature_sizes.gguf".to_string());
        let pytorch_input = Tensor::from_gguf_file(String::from("layer_norm_forward_different_feature_sizes_input"), &mut gguf_file);
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_forward_different_feature_sizes_output"), &mut gguf_file);
        let pytorch_weight = Tensor::from_gguf_file(String::from("layer_norm_forward_different_feature_sizes_weight"), &mut gguf_file);
        let pytorch_bias = Tensor::from_gguf_file(String::from("layer_norm_forward_different_feature_sizes_bias"), &mut gguf_file);
        let pytorch_mean = Tensor::from_gguf_file(String::from("layer_norm_forward_different_feature_sizes_mean"), &mut gguf_file);
        let pytorch_variance = Tensor::from_gguf_file(String::from("layer_norm_forward_different_feature_sizes_variance"), &mut gguf_file);
        let pytorch_std = Tensor::from_gguf_file(String::from("layer_norm_forward_different_feature_sizes_std"), &mut gguf_file);
        ...
        assert!(approx_equal(pytorch_mean_data[[0]], 5.5, 1e-4), "Mean should be 5.5: {}", pytorch_mean_data[[0]]);
        assert!(approx_equal(pytorch_variance_data[[0]], 8.25, 1e-4), "Variance should be 8.25: {}", pytorch_variance_data[[0]]);
        assert!(approx_equal(pytorch_std_data[[0]], 2.8722829818725586, 1e-4), "Std should be ~2.872: {}", pytorch_std_data[[0]]);
        ...
        assert!(rust_output[[0, 0]] < rust_output[[0, 1]], "Output should be increasing");
        assert!(rust_output[[0, 8]] < rust_output[[0, 9]], "Output should be increasing");
    ...
    fn test_layer_norm_forward_multiple_calls_pytorch_comparison() {
        ...
        // Load PyTorch test data from GGUF
        let mut gguf_file = GGUFFile::new("./models/tests/layer_norm/layer_norm_forward_multiple_calls.gguf".to_string());
        let pytorch_input = Tensor::from_gguf_file(String::from("layer_norm_forward_multiple_calls_input"), &mut gguf_file);
        let pytorch_output1 = Tensor::from_gguf_file(String::from("layer_norm_forward_multiple_calls_output1"), &mut gguf_file);
        let pytorch_output2 = Tensor::from_gguf_file(String::from("layer_norm_forward_multiple_calls_output2"), &mut gguf_file);
        let pytorch_weight = Tensor::from_gguf_file(String::from("layer_norm_forward_multiple_calls_weight"), &mut gguf_file);
        let pytorch_bias = Tensor::from_gguf_file(String::from("layer_norm_forward_multiple_calls_bias"), &mut gguf_file);
        let pytorch_mean = Tensor::from_gguf_file(String::from("layer_norm_forward_multiple_calls_mean"), &mut gguf_file);
        let pytorch_variance = Tensor::from_gguf_file(String::from("layer_norm_forward_multiple_calls_variance"), &mut gguf_file);
        let pytorch_std = Tensor::from_gguf_file(String::from("layer_norm_forward_multiple_calls_std"), &mut gguf_file);
        let pytorch_diff = Tensor::from_gguf_file(String::from("layer_norm_forward_multiple_calls_diff"), &mut gguf_file);
        ...
        for i in 0..3 {
            assert!(
                ...
                i, rust_output1[[0, i]], pytorch_output1_data[[0, i]]
            );
        }
        ...
        for i in 0..3 {
            assert!(
                ...
                i, rust_output2[[0, i]], pytorch_output2_data[[0, i]]
            );
        }
        ...
        for i in 0..3 {
            assert!(
                ...
                i, rust_output1[[0, i]], rust_output2[[0, i]]
            );
        }
        ...
        assert!(max_pytorch_diff < 1e-10, "PyTorch outputs should be identical, max diff: {}", max_pytorch_diff);
        ...
        let mean1 = sum1 / 3.0;
        let mean2 = sum2 / 3.0;
        assert!(approx_equal(mean1, 0.0, 1e-4), "Output1 mean should be ~0: {}", mean1);
        assert!(approx_equal(mean2, 0.0, 1e-4), "Output2 mean should be ~0: {}", mean2);
        
        // All values should be finite
        for i in 0..3 {
            assert!(rust_output1[[0, i]].is_finite(), "Output1 should be finite at [0, {}]", i);
            assert!(rust_output2[[0, i]].is_finite(), "Output2 should be finite at [0, {}]", i);
        }
        ...
        assert!(approx_equal(pytorch_mean_data[[0]], 2.0, 1e-4), "Mean should be 2.0: {}", pytorch_mean_data[[0]]);
        assert!(approx_equal(pytorch_variance_data[[0]], 0.6666666865348816, 1e-4), "Variance should be ~0.667: {}", pytorch_variance_data[[0]]);
        assert!(approx_equal(pytorch_std_data[[0]], 0.8165027499198914, 1e-4), "Std should be ~0.816: {}", pytorch_std_data[[0]]);
        
        // Verify expected output values: [-1.225, 0, 1.225]
        assert!(approx_equal(rust_output1[[0, 0]], -1.2247356176376343, 1e-4), "First output should be ~-1.225: {}", rust_output1[[0, 0]]);
        assert!(approx_equal(rust_output1[[0, 1]], 0.0, 1e-4), "Second output should be ~0: {}", rust_output1[[0, 1]]);
        assert!(approx_equal(rust_output1[[0, 2]], 1.2247356176376343, 1e-4), "Third output should be ~1.225: {}", rust_output1[[0, 2]]);
    ...
    fn test_layer_norm_backward_parameter_gradients_pytorch_comparison() {
        ...
        // Load PyTorch test data from GGUF
        let mut gguf_file = GGUFFile::new("./models/tests/layer_norm/layer_norm_backward_parameter_gradients.gguf".to_string());
        let pytorch_input = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_input"), &mut gguf_file);
        let pytorch_output = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_output"), &mut gguf_file);
        let pytorch_loss = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_loss"), &mut gguf_file);
        let pytorch_weight = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_weight"), &mut gguf_file);
        let pytorch_bias = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_bias"), &mut gguf_file);
        let pytorch_input_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_input_grad"), &mut gguf_file);
        let pytorch_weight_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_weight_grad"), &mut gguf_file);
        let pytorch_bias_grad = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_bias_grad"), &mut gguf_file);
        let pytorch_mean = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_mean"), &mut gguf_file);
        let pytorch_variance = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_variance"), &mut gguf_file);
        let pytorch_std = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_std"), &mut gguf_file);
        let pytorch_normalized = Tensor::from_gguf_file(String::from("layer_norm_backward_parameter_gradients_normalized"), &mut gguf_file);
        ...
        assert!(
            ...
            rust_loss[[0, 0]], pytorch_loss_data[0]
        );
        ...
        for i in 0..3 {
            assert!(
                ...
                "Weight gradient mismatch at [{}]: rust={}, pytorch={}",
                i, weight_grad[i], pytorch_weight_grad_data[i]
            );
        }
        ...
        for i in 0..3 {
            assert!(
                ...
                "Bias gradient mismatch at [{}]: rust={}, pytorch={}",
                i, bias_grad[i], pytorch_bias_grad_data[i]
            );
        }
        
        // Verify gradients are finite and non-zero where expected
        for i in 0..3 {
            assert!(weight_grad[i].is_finite(), "Weight gradient should be finite at [{}]", i);
            assert!(bias_grad[i].is_finite(), "Bias gradient should be finite at [{}]", i);
        }
        ...
        assert!(approx_equal(pytorch_mean_data[0], 4.0, 1e-4), "Mean should be 4.0: {}", pytorch_mean_data[0]);
        assert!(approx_equal(pytorch_variance_data[0], 6.0, 1e-4), "Variance should be 6.0: {}", pytorch_variance_data[0]);
        assert!(approx_equal(pytorch_std_data[0], 2.4494917392730713, 1e-4), "Std should be ~2.449: {}", pytorch_std_data[0]);
        
        // Verify normalized values match expected pattern
        assert!(approx_equal(pytorch_normalized_data[0], -1.2247356176376343, 1e-4), "First normalized should be ~-1.225: {}", pytorch_normalized_data[0]);
        assert!(approx_equal(pytorch_normalized_data[1], 0.0, 1e-4), "Second normalized should be ~0: {}", pytorch_normalized_data[1]);
        assert!(approx_equal(pytorch_normalized_data[2], 1.2247356176376343, 1e-4), "Third normalized should be ~1.225: {}", pytorch_normalized_data[2]);
        ...
        for i in 0..3 {
            assert!(
                ...
                i, weight_grad[i], pytorch_normalized_data[i]
            );
        }

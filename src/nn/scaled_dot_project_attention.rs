...
mod tests {
    ...
    fn test_scaled_dot_product_attention_backward_basic() {
        ...
        for i in 0..2 {
            for j in 0..2 {
                assert!(query_grad[[i, j]].is_finite(), "Query gradient should be finite");
                assert!(key_grad[[i, j]].is_finite(), "Key gradient should be finite");
                assert!(value_grad[[i, j]].is_finite(), "Value gradient should be finite");
            }
        }
    }
    ...
    fn test_scaled_dot_product_attention_backward_scaling() {
        ...
        assert!(gradients_differ, "Gradients should differ with different scaling factors");
    }
    ...
    fn test_scaled_dot_product_attention_backward_batch() {
        ...
        assert!(query_grad.iter().all(|&x| x.is_finite()), "All query gradients should be finite");
        assert!(key_grad.iter().all(|&x| x.is_finite()), "All key gradients should be finite");
        assert!(value_grad.iter().all(|&x| x.is_finite()), "All value gradients should be finite");
    }
}
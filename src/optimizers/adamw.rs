...
impl Optimizer for AdamW {
    ...
    fn update(&mut self) {
        for param in &self.params {
            ...
            let decayed_weights  : Vec<f32> = current_weight.iter().map(|theta_i|{
                *theta_i - self.learning_rate * self.weight_decay * theta_i
            }).collect();
            ...
            }).zip(decayed_weights).map(|(right_sidel, decayed_weight)|{
                decayed_weight - right_sidel
            }).collect();

            self.momentum.insert(*param, new_momentum);
            self.velocity.insert(*param, velocity_hat);

            get_equation().set_parameter(*param, right_side_of_update);
